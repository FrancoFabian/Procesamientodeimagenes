export class ImageType {
  ...
  */
  constructor(sc: CanvasRenderingContext2D, img?: HTMLImageElement, w?: number, h?: number, data?: boolean) {
    this._width = w || (img ? img.width : 0);
    this._height = h || (img ? img.height : 0);
    this.imageData = sc.getImageData(0, 0, this._width, this._height);
    this.screenCanvas = sc;

    // Inicializa el array arrImage
    this.initArray();

    // Llena el array arrImage con los datos de la imagen, si corresponde
    if (img && !data) {
